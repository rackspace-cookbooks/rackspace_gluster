...
# The simplest case
rackspace_gluster_repository 'test1' do
  description 'an test'
...
rackspace_gluster_repository 'test2' do
...
# create alias 'add'
rackspace_gluster_repository 'test3' do
  description 'an test'
  baseurl 'http://drop.the.baseurl.biz'
  action :add
end
...
# delete alias 'remove'
rackspace_gluster_repository 'test4' do
  description 'an test'
  baseurl 'http://drop.the.baseurl.biz'
  action :remove
end
...
# baseurl alias 'url'
rackspace_gluster_repository 'test5' do
  description 'an test'
...
# gpgkey alias 'keyurl'
rackspace_gluster_repository 'test6' do
  description 'an test'
  url 'http://drop.the.baseurl.biz'
...
rackspace_gluster_globalconfig '/tmp/gluster.conf' do
  action :create
end

# Full blown parameterization. Exercercises the
# recipe->resource->provider->template chain
rackspace_gluster_globalconfig '/tmp/gluster-full.conf' do
  alwaysprompt true
  assumeyes true
  bandwidth '40'
  ...
  password 'ohai'
  path '/tmp/gluster-full.conf'
  persistdir '/somewhere/good'
  pluginconfpath '/etc/gluster/pluginconf.d'
  pluginpath '/path/to /some/plugins'
  plugins true
  protected_multilib true
  protected_packages 'gluster glob:/etc/gluster/protected.d/*.conf'
